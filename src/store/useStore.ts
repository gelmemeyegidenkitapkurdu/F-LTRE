...
export interface Interview {
  ...
  interviewer: string;
  interviewee: string;
  interview_origin?: 'local' | 'foreign';
  dialogues: Dialogue[];
  created_at?: string;
  views?: number;
}
    ...
    (set, get) => ({
      ...
      fetchWritings: async () => {
        try {
          ...
          set({ writings: (data || []).map((w: any) => ({ ...w, status: w.status || "published", content: w.content || "" })) });
        } catch (error) {
      ...
      fetchBookPdf: async (id: string) => {
        const existingPdf = get().books.find((item) => item.id === id)?.pdf ?? null;

        try {
          const pdf = await api.fetchBookPdf(id);
          const resolvedPdf = pdf ?? existingPdf;

          if (resolvedPdf && resolvedPdf !== existingPdf) {
            set((state) => ({
              books: state.books.map((item) => (item.id === id ? { ...item, pdf: resolvedPdf } : item)),
            }));
          }
      ...
      addBook: async (book) => {
        ...
        try {
          ...
          const refreshed = await api.fetchBooks();
          set({ books: (refreshed || []).map((b: any) => ({ ...b, pdf: b.pdf ?? null })) });
        } catch (error) {
      ...
      addPoll: async (poll) => {
        ...
        } as Poll & { created_at?: string };

        set((state) => ({ polls: [optimisticPoll, ...state.polls] }));
        ...
        try {
          await api.createPoll({
            ...
            views: poll.views ?? 0,
            options,
            created_at: optimisticPoll.created_at,
          });
      ...
      addInterview: async (interview) => {
        ...
        const optimisticInterview = { ...interview, id, dialogues, created_at };
        set((state) => ({ interviews: [optimisticInterview, ...state.interviews] }));
      ...
      updateInterview: async (id, interview) => {
        ...
        try {
          ...
          set({ interviews: previousInterviews });
          throw error;
        }
      },
