...
export const MainLayout = () => {
  ...
  return (
    <div
      className={`min-h-screen min-h-[100svh] min-h-[100dvh] flex flex-col font-serif relative overflow-x-hidden transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-200' : 'bg-pink-50/30 text-gray-800'}`}
      style={{
    ...
    >
      ...
      <header className={`relative backdrop-blur-sm shadow-sm z-10 transition-colors duration-300 ${darkMode ? 'bg-gray-800/90' : 'bg-white/90'}`}>
        <div className="container mx-auto px-4 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-6 flex flex-col items-center justify-center relative">
      ...
      {/* Main Content */}
      <main className="container mx-auto px-4 py-8 relative flex-1 w-full">
        <Outlet />
      </main>
